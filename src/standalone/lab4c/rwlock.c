...
struct inode
{
  ...
  // Add any other members you need for your readers-writers lock here.
    struct lock write_lock;             /* Lock for writing to inode */
    struct condition write_cond;        /* Condition variable for writing */
    bool writing;                       /* True if writing to inode */
...
struct inode *inode_open(void) NO_STEP
{
  ...
  /* Initialize. */
  node->readers = 0;
  lock_init(&node->write_lock);
  cond_init(&node->write_cond);
  node->writing = false;
...
void inode_read_at(struct inode *inode, int *output)
{
  ...
  *output = inode->data;
  ...
  lock_acquire(&inode->write_lock);
  --inode->readers;
  // Signal writing to continue
  if (inode->readers == 0) cond_broadcast(&inode->write_cond, &inode->write_lock);
  lock_release(&inode->write_lock);;
}
...
void inode_write_at(struct inode *inode, int *input)
{
  ...
  // the visualization tool. Add your rw-lock before and after.
  lock_acquire(&inode->write_lock);
  // Wait for writing or reading to finish
  while (inode->writing || inode->readers) {
    cond_wait(&inode->write_cond, &inode->write_lock);
    }
  inode->writing = true;
  lock_release(&inode->write_lock);
